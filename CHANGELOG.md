...
- **SR announcement simulator** — heuristic prediction of what NVDA, JAWS, and VoiceOver each announce for all target kinds (landmarks, headings, controls, form fields, dialogs, status messages, menus, tabs). State-aware: emits "Subscribe, check box, checked", "Country, combo box, collapsed", "Actions, menu button, collapsed", "Volume, slider, 75", "Email, edit, invalid entry, required, you must use a work address" (with `aria-describedby` resolved), etc. Encodes documented cross-AT differences (NVDA "edit" vs VoiceOver "text field"; VoiceOver "popup button" for `<select>`-style comboboxes). **Calibrated against [W3C ARIA-AT](https://aria-at.w3.org)**: passes per-target assertions at 100% across all three ATs for 15 tested patterns (button, checkbox, switch, slider, dialog, tabs, link, alert, disclosure, menu button, spin button, main, banner, contentinfo, complementary). Run `npm run calibrate` to verify. Exported from `tactual/playwright`.
