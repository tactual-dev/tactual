...
- **SR announcement simulator** — heuristic prediction of what NVDA, JAWS, and VoiceOver each announce for all target kinds (landmarks, headings, controls, form fields, dialogs, status messages, menus, tabs). State-aware: emits "Subscribe, check box, checked", "Country, combo box, collapsed", "Volume, slider, 75", "Email, edit, invalid entry, required, you must use a work address" (with `aria-describedby` resolved), etc. Encodes documented cross-AT differences (NVDA "edit" vs VoiceOver "text field"; VoiceOver "popup button" for `<select>`-style comboboxes). **Data quality:** the simulator is heuristic prediction, not verified test output. The role map ships with confidence labels (HIGH/MEDIUM/LOW) and falls through to a shared base when uncertain rather than inventing AT-specific differences. See `src/calibration/aria-at.ts` for a path to verify against W3C ARIA-AT ground truth. Exported from `tactual/playwright`.
...
- **Navigation transcript** — new `tactual transcript <url> [--at nvda|jaws|voiceover]` command prints the linear sequence of announcements an SR user hears as they Tab through a page. Text and JSON output formats.
